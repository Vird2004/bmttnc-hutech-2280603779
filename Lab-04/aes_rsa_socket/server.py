...
def handle_client(client_socket, client_address):
    ...
    while True:
        ...
        # Gửi tin nhắn nhận được cho các client khác
        for client, key in clients:
            if client != client_socket:
                encrypted = encrypt_message(key, decrypted_message)
                client.send(encrypted)
            # Nếu tin nhắn giải mã được là "exit", thoát vòng lặp
            if decrypted_message == "exit":
                break

        # Loại client khỏi danh sách và đóng kết nối
    clients.remove((client_socket, aes_key))
    client_socket.close()
    print(f"Đã ngắt kết nối với {client_address}")

# Chấp nhận và xử lý kết nối từ các client
while True:
    client_socket, client_address = server_socket.accept()
    print(f"Kết nối mới từ {client_address}")
    # Tạo luồng riêng để xử lý mỗi kết nối client
